        ...
        void IHeartbeat.OnHeartbeatUpdate(TimeSpan dif)
        {
            switch (State)
            {
                ...
                case States.Open:
                    ...
                    if ((LastPingSent != DateTime.MinValue) && (LastPongReceived == DateTime.MinValue) && (DateTime.UtcNow - LastPingSent > Handshake.PingTimeout))
                    {
                        UnityEngine.Debug.Log(string.Format("{0} - {1} - {2}", LastPingSent, LastPongReceived, Handshake.PingTimeout));
                        (this as IManager).TryToReconnect();
                    }
